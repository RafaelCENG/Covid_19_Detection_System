...
router.get("/visit_history", authController.isLoggedIn, (req, res) => {
  if (req.user) {
    ...
    db.query(
      "SELECT name_of_pois,Timestamp from pois_visit where user_id = ?",
      userid,
      function (error, results, feilds) {
        if (error) {
          console.log(
            "error ocurred while getting user details of " + userid,
            error
          )
          res.send({
            code: 400,
...
router.get("/case_history", authController.isLoggedIn, (req, res) => {
  if (req.user) {
    ...
    db.query(
      "SELECT time from confirmed_Case where username = ?",
      username,
      function (error, results, fields) {
        if (error) {
          console.log(
            "error ocurred while getting user details of " + username,
            error
          )
          res.send({
            code: 400,
